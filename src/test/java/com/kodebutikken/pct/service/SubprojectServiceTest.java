...
class SubprojectServiceTest {
    ...
    void createSubproject_success() {
        ...
        when(projectRepository.getProjectById(1)).thenReturn(new Project());

        subprojectService.createSubproject(1, form);

        ArgumentCaptor<Subproject> captor =
                ArgumentCaptor.forClass(Subproject.class);
    ...
    @Test
    void createSubproject_shouldThrowException_whenProjectDoesNotExist() {

        SubprojectForm form = new SubprojectForm();
        form.setTitle("Subproject");
        form.setDeadline(LocalDate.now().plusDays(5));
        ...
        verify(subprojectRepository, never()).createSubproject(any());
    }
